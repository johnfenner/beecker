...
def load_pipeline_data():
    ...
    try:
        ...
        try:
            ...
            st.error(f"Error: No se pudo encontrar la pestaña llamada '{PIPELINE_SHEET_NAME}'.")
            st.info("Verifica que la pestaña exista y el nombre sea exacto.")
            st.stop()
        ...
        if not raw_data or len(raw_data) <= 1:
            st.error("La hoja del Pipeline está vacía o no tiene encabezados.")
            return pd.DataFrame()
        ...
        if "This operation is not supported for this document" in str(e):
            st.error(f"Error al leer la hoja: Aún es un archivo Excel (.xlsx).")
            st.warning("Asegúrate de haber 'Guardado como Hoja de Google' y estar usando la *nueva* URL en 'secrets.toml'.")
        else:
            st.error(f"Error de API al leer la hoja: {e}")
        st.stop()
    except Exception as e:
        st.error(f"Error general al leer la hoja: {e}")
        st.stop()
    ...
    if COL_PRIMARY_DATE in df.columns:
        ...
        st.error(f"Columna '{COL_PRIMARY_DATE}' no encontrada.")
        st.stop()
...
def apply_pipeline_filters(df, start_dt, end_dt, industries, managements, meeting_status):
    ...
    if "Fecha_Principal" in df_f.columns:
        ...
        if start_dt_date and end_dt_date:
            valid_indices = fecha_series_valid[fecha_series_valid.between(start_dt_date, end_dt_date, inclusive="both")].index
            df_f = df_f.loc[valid_indices]
        elif start_dt_date:
            valid_indices = fecha_series_valid[fecha_series_valid >= start_dt_date].index
            df_f = df_f.loc[valid_indices]
        elif end_dt_date:
             valid_indices = fecha_series_valid[fecha_series_valid <= end_dt_date].index
             df_f = df_f.loc[valid_indices]
...
if not df_pipeline_filtered.empty:
    ...
    with st.expander("Ver tabla de datos detallados del pipeline (filtrada)"):
        ...
        if 'Fecha_Principal' in df_display.columns: 
        ...
        if COL_MEETING_DATE in df_display.columns:
             df_display[COL_MEETING_DATE] = df_display[COL_MEETING_DATE].dt.strftime('%Y-%m-%d').fillna('')
        ...
        for col in cols_to_show:
             ...
             elif col == COL_PRIMARY_DATE and COL_PRIMARY_DATE in df_display.columns: 
