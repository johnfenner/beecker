...
# El código original no se ha modificado. Se han añadido nuevas funciones al final.
...
def display_campaign_filters(df_options):
    ...
    if not isinstance(current_campaign_selection_from_state, list):
        campaign_selection_is_valid = False
    else:
        for item in current_campaign_selection_from_state:
    ...
    if not prospector_selection_is_valid:
        ...
        st.rerun()
    selected_prospectors = st.sidebar.multiselect("¿Quién Prospectó?", options=prospector_options, key=SES_PROSPECTOR_FILTER_KEY)
    ...
    if not avatar_selection_is_valid:
        ...
        st.rerun()
    selected_avatars = st.sidebar.multiselect("Avatar", options=avatar_options, key=SES_AVATAR_FILTER_KEY)
    ...
    st.sidebar.markdown("---")
    if st.sidebar.button("🧹 Limpiar Filtros", use_container_width=True, key=f"{SES_CAMPAIGN_FILTER_KEY}_clear_button_final_v3"):
        st.session_state[SES_CAMPAIGN_FILTER_KEY] = default_filters_init[SES_CAMPAIGN_FILTER_KEY]
...
def load_email_campaign_stats():
    ...
    try:
        ...
        if not sheet_url:
            st.warning("La URL para la hoja de estadísticas de email ('email_stats_sheet_url') no está en tus secrets. No se puede cargar la sección de 'Números por correo'.")
            return None, None
        ...
        for i, row in enumerate(df[0]):
            if str(row).strip() == "Tabla_1":
                tabla1_start_row = i
            elif str(row).strip() == "Tabla_2":
                tabla2_start_row = i
...
st.markdown("---")
with st.expander("📊 Análisis de Rendimiento por Correo Campaña (Hoja Nueva)"):
    # Cargar los datos de las nuevas tablas de forma independiente
