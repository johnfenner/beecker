...
def generar_tabla_comparativa_campanas_filtrada(df_filtrado_con_filtros_pagina, lista_nombres_campanas_seleccionadas, df_original_completo):
    datos_comparativa = []
    ...
    for nombre_campana in lista_nombres_campanas_seleccionadas:
        ...
        kpis_filtrados = calcular_kpis_df_campana(df_campana_individual_filtrada)

        # Datos originales de la hoja para esta campaña (sin ningún filtro aplicado)
        df_campana_original = df_original_completo[
            df_original_completo['Campaña'] == nombre_campana
        ]
        total_prospectos_original = len(df_campana_original) if not df_campana_original.empty else 0

        datos_comparativa.append({
            "Campaña": nombre_campana,
            "Prospectos Originales (Sheet)": total_prospectos_original, # Nuevo campo
            "Prospectos (Aplicados Filtro)": kpis_filtrados["total_prospectos"],
            "Aceptadas (Aplicadas Filtro)": kpis_filtrados["invites_aceptadas"],
            "Respuestas (Aplicadas Filtro)": kpis_filtrados["respuestas_primer_mensaje"],
            "Sesiones (Aplicadas Filtro)": kpis_filtrados["sesiones_agendadas"],
            "Tasa Aceptación (%)": kpis_filtrados["tasa_aceptacion"],
            "Tasa Respuesta (vs Acept.) (%)": kpis_filtrados["tasa_respuesta_vs_aceptadas"],
            "Tasa Sesiones (vs Resp.) (%)": kpis_filtrados["tasa_sesion_vs_respuesta"],
            "Tasa Sesión Global (%)": kpis_filtrados["tasa_sesion_global"]
        })
    return pd.DataFrame(datos_comparativa)
...
if df_final_analisis_campana.empty:
    ...
    if len(st.session_state.campana_seleccion_principal) > 0: # Cambiado de > 1 a > 0 para que siempre muestre la tabla si hay campaña seleccionada
        st.markdown("---")
        st.header(f"🔄 Comparativa Detallada entre Campañas (con y sin filtros de página)")
        st.caption("La siguiente tabla compara las campañas seleccionadas, mostrando los prospectos totales de la Google Sheet original y los KPIs afectados por los filtros de '¿Quién Prospectó?', 'País' y 'Fechas' aplicados arriba.")
        df_tabla_comp = generar_tabla_comparativa_campanas_filtrada(df_final_analisis_campana, st.session_state.campana_seleccion_principal, df_original_completo)
        if not df_tabla_comp.empty:
            st.subheader("Tabla Comparativa de KPIs")
            # Ajustar las columnas enteras para el nuevo campo
            cols_enteros_comp = ["Prospectos Originales (Sheet)", "Prospectos (Aplicados Filtro)", "Aceptadas (Aplicadas Filtro)", "Respuestas (Aplicadas Filtro)", "Sesiones (Aplicadas Filtro)"]
            ...
            # Los gráficos de comparativa solo tienen sentido si hay más de una campaña para comparar
            if len(st.session_state.campana_seleccion_principal) > 1:
                ...
                df_graf_comp_tasa_global = df_tabla_comp[df_tabla_comp["Prospectos (Aplicados Filtro)"] > 0].sort_values(by="Tasa Sesión Global (%)", ascending=False)
                ...
                df_graf_comp_vol_sesiones = df_tabla_comp[df_tabla_comp["Sesiones (Aplicadas Filtro)"] > 0].sort_values(by="Sesiones (Aplicadas Filtro)", ascending=False)
                if not df_graf_comp_vol_sesiones.empty:
                    fig_comp_vol = px.bar(df_graf_comp_vol_sesiones, x="Campaña", y="Sesiones (Aplicadas Filtro)", title="Comparativa: Volumen de Sesiones Agendadas", text="Sesiones (Aplicadas Filtro)", color="Campaña")
                ...
                st.info("Selecciona más de una campaña para ver los gráficos de comparativa detallada.")
    ...
    if "¿Quién Prospecto?" in df_final_analisis_campana.columns:
        ...
        if not df_prospectador_camp_display.empty:
            st.dataframe(df_prospectador_camp_display.style.format(format_prosp), use_container_width=True, hide_index=True)
