        ...
        if not categorias_reales_con_plantillas_en_df and not ("General" in opciones_mensajes and opcion_todas_las_categorias in categorias_seleccionables_para_widget):
            ...
            if mensaje_final_seleccionado:
                ...
                if df_vista_previa_msg.empty:
                    ...
                    st.markdown("💡 **Haz click en una fila para ver el mensaje completo y copiarlo fácilmente.**")
                   ...
                   # ... (código anterior hasta la parte de mostrar la tabla seleccionable) ...
                    ...
                    if selected_rows_data and selected_rows_data.get("selection") and selected_rows_data["selection"].get("rows"):
                        ...
                        st.markdown("---")
                        st.subheader(f"Mensaje completo para {selected_prospect.get('Nombre_Completo_Display', 'prospecto')}:")
                        ...
                        st.text_area(
                            ...
                            height=250,
                            key=f"selected_message_copy_{selected_index}"
                        )
                    else:
