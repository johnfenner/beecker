...
with st.expander("Ver/Ocultar Filtros Adicionales"):
    ...
    with col1_filtros:
        ...
        st.session_state.mensaje_filtros["fuente_lista"] = st.multiselect("Fuente de la Lista", opciones_fuente, default=st.session_state.mensaje_filtros.get("fuente_lista", ["– Todos –"]), key="ms_fuente_lista_msg_page_v3") # Key actualizada
        ...
        st.session_state.mensaje_filtros["proceso"] = st.multiselect("Proceso", opciones_proceso, default=st.session_state.mensaje_filtros.get("proceso", ["– Todos –"]), key="ms_proceso_msg_page_v3") # Key actualizada
        ...
        st.session_state.mensaje_filtros["avatar"] = st.multiselect("Avatar", avatares_unicos_filt, default=st.session_state.mensaje_filtros.get("avatar", ["– Todos –"]), key="ms_avatar_msg_page_v3") # Key actualizada

    with col2_filtros:
        opciones_pais = ["– Todos –"] + (sorted(df["Pais"].dropna().astype(str).unique().tolist()) if "Pais" in df.columns and not df["Pais"].empty else [])
        st.session_state.mensaje_filtros["pais"] = st.multiselect("País", opciones_pais, default=st.session_state.mensaje_filtros.get("pais", ["– Todos –"]), key="ms_pais_msg_page_v3") # Key actualizada
        ...
        st.session_state.mensaje_filtros["industria"] = st.multiselect("Industria", opciones_industria, default=st.session_state.mensaje_filtros.get("industria", ["– Todos –"]), key="ms_industria_msg_page_v3") # Key actualizada
        ...
        st.session_state.mensaje_filtros["prospectador"] = st.multiselect("¿Quién Prospectó?", opciones_prospectador, default=st.session_state.mensaje_filtros.get("prospectador", ["– Todos –"]), key="ms_prospectador_msg_page_v3") # Key actualizada
    ...
    with st.container():
        ...
        with col_sesion_filt:
            ...
            st.session_state.mensaje_filtros["sesion_agendada"] = st.selectbox("¿Sesión Agendada?", opciones_sesion_filt, index=opciones_sesion_filt.index(current_sesion_val_filt), key="sb_sesion_agendada_msg_page_v3") # Key actualizada

        with col_f1_filt:
            st.session_state.mensaje_filtros["fecha_ini"] = st.date_input("Desde (Fecha Primer Mensaje)", value=st.session_state.mensaje_filtros.get("fecha_ini", None), format='DD/MM/YYYY', key="di_fecha_ini_msg_page_v3", min_value=fecha_min_data_val, max_value=fecha_max_data_val) # Key actualizada
        with col_f2_filt:
            st.session_state.mensaje_filtros["fecha_fin"] = st.date_input("Hasta (Fecha Primer Mensaje)", value=st.session_state.mensaje_filtros.get("fecha_fin", None), format='DD/MM/YYYY', key="di_fecha_fin_msg_page_v3", min_value=fecha_min_data_val, max_value=fecha_max_data_val) # Key actualizada

st.session_state.mensaje_filtros["busqueda"] = st.text_input("🔎 Buscar en Nombre, Apellido, Empresa, Puesto", value=st.session_state.mensaje_filtros.get("busqueda", ""), placeholder="Ingrese término y presione Enter", key="ti_busqueda_msg_page_v3") # Key actualizada

col_btn1, col_btn2 = st.columns(2)
with col_btn1:
    if st.button("📬 Cargar y Filtrar Prospectos para Mensaje", key="btn_cargar_filtrar_msg_page_v3"): # Key actualizada
        st.session_state.mostrar_tabla_mensajes = True
with col_btn2:
    st.button("🧹 Limpiar Filtros de Mensajes", on_click=reset_mensaje_filtros_state, key="btn_limpiar_filtros_msg_page_v3") # Key actualizada
...
if st.session_state.mostrar_tabla_mensajes:
    ...
    if df_mensajes_final_display.empty:
        ...
        columnas_necesarias_para_display = [
            ...
            columna_fecha_a_mostrar, 
            "¿Quién Prospecto?", linkedin_col_nombre, "Sesion Agendada?"
        ]
        ...
        if columna_fecha_a_mostrar in df_tabla_a_mostrar.columns:
            if not pd.api.types.is_datetime64_any_dtype(df_tabla_a_mostrar[columna_fecha_a_mostrar]):
                df_tabla_a_mostrar[columna_fecha_a_mostrar] = pd.to_datetime(df_tabla_a_mostrar[columna_fecha_a_mostrar], errors='coerce')
        ...
        if not categorias_reales_con_plantillas_en_df:
            st.warning("No hay prospectos con categorías válidas que tengan plantillas definidas en los datos filtrados.")
        else:
            ...
            with col_sel_cat:
                categoria_sel_widget = st.selectbox(
                    ...
                    index=default_categoria_index, # Mantener selección previa si es válida
            ...
            with col_sel_plantilla:
                ...
                if nombres_plantillas_para_categoria_sel:
                    ...
                    if not (categoria_sel_widget == opcion_todas_las_categorias and not ("General" in opciones_mensajes)):
                         st.warning(f"No hay plantillas definidas para la categoría '{categoria_usada_para_plantillas if categoria_usada_para_plantillas else categoria_sel_widget}'.")
            ...
            if mensaje_final_seleccionado:
                ...
                if df_vista_previa_msg.empty:
                    st.info(f"No hay prospectos en '{categoria_sel_widget}' con los filtros actuales para generar mensajes.")
                else:
                    def obtener_atencion_genero(avatar_de_fila):
                        avatar_estandarizado_lower = str(avatar_de_fila).lower()
                        nombres_masculinos_clave = ["john bermúdez", "johnsito"] 
                    ...
                    @st.cache_data
                    def convert_df_to_csv_final(df_to_convert_csv): # Nombre de variable diferente
                        cols_descarga = ["Nombre_Completo_Display", "Empresa", "Puesto", "Sesion Agendada?", linkedin_col_nombre, "Mensaje_Personalizado"]
                    ...
                    if nombre_archivo_cat_final == opcion_todas_las_categorias : nombre_archivo_cat_final = "todas_categorias" # fallback si algo raro pasa

                    if csv_data_final is not None:
                        st.download_button(
                            label="⬇️ Descargar Mensajes Generados (CSV)",
                            data=csv_data_final,
                            file_name=f'mensajes_{nombre_archivo_cat_final.replace(" ", "_").lower()}_{nombre_plantilla_sel.replace(" ", "_").lower()}.csv',
                            mime='text/csv',
    ...
    if df_vista_libre.empty:
        ...
        df_vista_libre["Mensaje_Personalizado"] = df_vista_libre.apply(
            lambda row: mensaje_base_libre
                ...
                .replace("[Nombre de la empresa]", str(row.get("Empresa", "[Nombre de la empresa]")))
                .replace("{atencion_genero}", obtener_atencion_genero(row.get("Avatar")))
                .replace(". ", ".\n\n")  # mejora el espaciado para LinkedIn
                .replace(": ", ":\n"),    # mejora visual para listas tipo bullets
            axis=1
        )
