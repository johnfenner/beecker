...
def make_unique_headers(headers_list):
    ...
    for h in headers_list:
        ...
        if not h_stripped: h_stripped = "Columna_Vacia" # Nombre por defecto para encabezados vacíos
...
def parse_date_robust(date_val):
    ...
    for fmt in ("%d/%m/%Y %H:%M:%S", "%d/%m/%Y %H:%M", "%d/%m/%Y", "%Y-%m-%d %H:%M:%S", "%Y-%m-%d", "%m/%d/%Y %H:%M:%S", "%m/%d/%Y"):
        ...
        except: continue # No uses ValueError, TypeError aquí para no enmascarar otros errores
    try: return pd.to_datetime(date_str, errors='coerce')
    except: return pd.NaT # No uses ValueError, TypeError aquí

def separar_nombre_cargo_suramerica(nombre_cargo_str):
    nombre, apellido, puesto = pd.NA, pd.NA, "No Especificado"
    if pd.isna(nombre_cargo_str) or not isinstance(nombre_cargo_str, str) or not nombre_cargo_str.strip(): return nombre, apellido, puesto
    nombre_cargo_str = nombre_cargo_str.strip()
    delimiters_cargo = [' - ', ' / ', ', ', ' – ']; nombre_completo_str = nombre_cargo_str; cargo_encontrado_explicitamente = False
    for delim in delimiters_cargo:
        if delim in nombre_cargo_str:
            parts = nombre_cargo_str.split(delim, 1); nombre_completo_str = parts[0].strip()
            if len(parts) > 1 and parts[1].strip(): puesto = parts[1].strip(); cargo_encontrado_explicitamente = True
            break
    ...
    if len(name_parts) == 1: nombre = name_parts[0]
        ...
        if not cargo_encontrado_explicitamente:
            temp_nombre_simple, temp_apellido_simple = name_parts[0], name_parts[1]; temp_cargo_implicito = " ".join(name_parts[2:])
            if len(temp_cargo_implicito) > 3 and len(temp_cargo_implicito.split()) > 1: nombre, apellido, puesto = temp_nombre_simple, temp_apellido_simple, temp_cargo_implicito
...
def load_sesiones_data():
    ...
    try:
        ...
        if raw_data_principal_list and len(raw_data_principal_list) > 1:
            ...
            df_proc_principal["Fecha"] = df_principal_raw.get("Fecha"); df_proc_principal["Empresa"] = df_principal_raw.get("Empresa")
            df_proc_principal["País"] = df_principal_raw.get("País"); df_proc_principal["Nombre"] = df_principal_raw.get("Nombre")
            df_proc_principal["Apellido"] = df_principal_raw.get("Apellido"); df_proc_principal["Puesto"] = df_principal_raw.get("Puesto")
            df_proc_principal["SQL"] = df_principal_raw.get("SQL"); df_proc_principal["AE"] = df_principal_raw.get("AE")
            df_proc_principal["LG"] = df_principal_raw.get("LG"); df_proc_principal["Siguientes Pasos"] = df_principal_raw.get("Siguientes Pasos")
            df_proc_principal["Email"] = df_principal_raw.get("Email"); df_proc_principal["RPA"] = df_principal_raw.get("RPA")
            df_proc_principal["LinkedIn"] = df_principal_raw.get("LinkedIn"); df_proc_principal["Fuente_Hoja"] = "Principal"
            all_dataframes.append(df_proc_principal)
    except Exception as e: print(f"ADVERTENCIA al cargar Hoja Principal: {e}")
    ...
    try:
        ...
        if not df_suramerica_raw.empty:
            ...
            df_suramerica_processed["Fecha"] = df_suramerica_raw.get("Fecha"); df_suramerica_processed["Empresa"] = df_suramerica_raw.get("Empresa")
            df_suramerica_processed["País"] = df_suramerica_raw.get("País"); df_suramerica_processed["Siguientes Pasos"] = df_suramerica_raw.get("Siguientes Pasos")
            df_suramerica_processed["SQL"] = df_suramerica_raw.get("SQL"); df_suramerica_processed["Email"] = df_suramerica_raw.get("Correo")
            df_suramerica_processed["LinkedIn"] = df_suramerica_raw.get("LinkedIn"); df_suramerica_processed["LG"] = df_suramerica_raw.get("LG")
            ...
            if "Nombre y Cargo" in df_suramerica_raw.columns:
                ...
                df_suramerica_processed["Nombre"] = n_c_split.apply(lambda x: x[0]); df_suramerica_processed["Apellido"] = n_c_split.apply(lambda x: x[1])
                df_suramerica_processed["Puesto"] = n_c_split.apply(lambda x: x[2])
            else: df_suramerica_processed["Nombre"], df_suramerica_processed["Apellido"], df_suramerica_processed["Puesto"] = pd.NA, pd.NA, "No Especificado"
            df_suramerica_processed["Fuente_Hoja"] = "Suramérica"
            all_dataframes.append(df_suramerica_processed)
    except Exception as e: print(f"ADVERTENCIA al cargar Hoja Suramérica: {e}")

    if not all_dataframes: return df_final_structure
    df_consolidado = pd.concat(all_dataframes, ignore_index=True, sort=False)
    if "Fecha" not in df_consolidado.columns or df_consolidado["Fecha"].isnull().all(): return df_final_structure
    df_consolidado["Fecha"] = df_consolidado["Fecha"].apply(parse_date_robust)
    df_consolidado.dropna(subset=["Fecha"], inplace=True)
    if df_consolidado.empty: return df_final_structure

    df_procesado = df_consolidado.copy()
    try:
        df_procesado['Año'] = df_procesado['Fecha'].dt.year.astype('Int64'); df_procesado['NumSemana'] = df_procesado['Fecha'].dt.isocalendar().week.astype('Int64')
        df_procesado['MesNombre'] = df_procesado['Fecha'].dt.month_name(); df_procesado['AñoMes'] = df_procesado['Fecha'].dt.strftime('%Y-%m')
    except Exception as e_time: print(f"Error creando columnas de tiempo: {e_time}"); return df_final_structure

    default_values_fill = {"AE": "No Asignado AE", "LG": "No Asignado LG", "Puesto": "No Especificado", "Empresa": "No Especificado", "País": "No Especificado", "Nombre": "No Especificado", "Apellido": "No Especificado", "Siguientes Pasos": "No Especificado", "Email": "No Especificado", "RPA": "No Aplicable", "LinkedIn": "No Especificado", "Fuente_Hoja": "Desconocida", "SQL": "SIN CALIFICACIÓN SQL", "SQL_Estandarizado": "SIN CALIFICACIÓN SQL"}
    ...
    for col in COLUMNAS_CENTRALES:
        if col in ["SQL", "SQL_Estandarizado"]:
            ...
            df_procesado[col] = df_procesado[col].fillna(default_values_fill[col]); continue
        if col in df_procesado.columns:
            default_val = default_values_fill.get(col, "No Especificado"); df_procesado[col] = df_procesado[col].astype(str)
            df_procesado[col] = df_procesado[col].replace(generic_empty_na_values, default_val, regex=False)
            df_procesado[col] = df_procesado[col].str.strip(); df_procesado.loc[df_procesado[col] == '', col] = default_val
            df_procesado[col] = df_procesado[col].fillna(default_val)
        elif col not in df_procesado.columns and col not in ['Fecha', 'Año', 'NumSemana']: df_procesado[col] = default_values_fill.get(col, "No Disponible")
    ...
    mask_empty_sql = ~sql_estandarizado_str.isin(known_sql_values) & (sql_estandarizado_str.isin(['', 'NAN', 'NONE', '<NA>', 'N/A']))
    ...
    for col in COLUMNAS_CENTRALES:
        if col in df_procesado.columns: df_final_filtrado[col] = df_procesado[col]
        else:
    ...
    try:
        if 'Fecha' in df_final_filtrado.columns: df_final_filtrado['Fecha'] = pd.to_datetime(df_final_filtrado['Fecha'], errors='coerce')
        if 'Año' in df_final_filtrado.columns: df_final_filtrado['Año'] = pd.to_numeric(df_final_filtrado['Año'], errors='coerce').astype('Int64')
        if 'NumSemana' in df_final_filtrado.columns: df_final_filtrado['NumSemana'] = pd.to_numeric(df_final_filtrado['NumSemana'], errors='coerce').astype('Int64')
        for col_name in df_final_filtrado.columns:
            if col_name not in ['Fecha', 'Año', 'NumSemana'] and col_name in df_final_filtrado.columns:
                df_final_filtrado[col_name] = df_final_filtrado[col_name].astype(str)
    except Exception as e_type: print(f"ADVERTENCIA al ajustar tipos finales: {e_type}")
    return df_final_filtrado

# ==============================================================================
# COPIA Y PEGA AQUÍ EL RESTO DE TU CÓDIGO:
# Todas tus funciones desde clear_ses_filters_callback()
# hasta el final del script (el flujo principal que llama a load_sesiones_data()
# y luego a las funciones de display).
# ==============================================================================

# Ejemplo de cómo debería continuar tu código:
...
def sidebar_filters_sesiones(df_options):
    ...
    elif not validated_week_selection and weeks_options: validated_week_selection = [weeks_options[0]] if weeks_options and weeks_options[0] != "– Todas –" else []
    ...
    return (st.session_state[SES_START_DATE_KEY], st.session_state[SES_END_DATE_KEY], sel_y, st.session_state[SES_WEEK_FILTER_KEY], st.session_state[SES_AE_FILTER_KEY], st.session_state[SES_LG_FILTER_KEY], st.session_state[SES_PAIS_FILTER_KEY], st.session_state[SES_SQL_FILTER_KEY])
...
def apply_sesiones_filters(df, start_date, end_date, year_f, week_f, ae_f, lg_f, pais_f, sql_f):
    ...
    if year_f is not None and "Año" in df_f.columns and pd.api.types.is_integer_dtype(df_f["Año"]): df_f = df_f[df_f["Año"] == year_f]
...
def display_sesiones_summary_sql(df_filtered):
    ...
    if 'SQL_Estandarizado' in df_filtered.columns:
        ...
        if not sql_counts.empty:
            ...
            fig_sql_summary.update_layout(xaxis={'categoryorder':'array', 'categoryarray':category_order_sql_summary}); st.plotly_chart(fig_sql_summary, use_container_width=True)
...
def display_analisis_por_dimension(df_filtered, dimension_col, dimension_label, top_n=10):
    ...
    dim_totals = df_filtered[dimension_col].value_counts().head(top_n); top_n_dims_list = dim_totals.index.tolist()
    ...
    fig_dim_analysis.update_layout(xaxis_tickangle=-45, yaxis_title="Número de Sesiones", xaxis={'categoryorder':'array', 'categoryarray':top_n_dims_list}); st.plotly_chart(fig_dim_analysis, use_container_width=True)
    try:
        ...
        pivot_table_dim = pivot_table_dim.reindex(columns=sql_category_order_dim_analysis, fill_value=0); pivot_table_dim = pivot_table_dim.reindex(index=top_n_dims_list, fill_value=0)
        pivot_table_dim['Total_Sesiones_Dim'] = pivot_table_dim.sum(axis=1); format_dict_dim = {col: "{:,.0f}" for col in pivot_table_dim.columns}
...
def display_evolucion_sql(df_filtered, time_agg_col, display_label, chart_title, x_axis_label):
    ...
    df_agg_evol = df_filtered.copy(); group_col_evol = time_agg_col; sort_col_evol = time_agg_col
    ...
    try:
        ...
        fig_evol_sql.update_xaxes(type='category'); fig_evol_sql.update_layout(xaxis_title=x_axis_label); st.plotly_chart(fig_evol_sql, use_container_width=True)
...
def display_tabla_sesiones_detalle(df_filtered):
    ...
    cols_display_detalle_ses = ["Fecha", "LG", "AE", "País", "SQL", "SQL_Estandarizado", "Empresa", "Puesto", "Nombre", "Apellido", "Siguientes Pasos", "RPA", "Fuente_Hoja", "LinkedIn"]
    ...
    if "Fecha" in df_view_detalle_ses.columns and pd.api.types.is_datetime64_any_dtype(df_view_detalle_ses["Fecha"]):
         ...
         except AttributeError: pass # Mantener como está si no se puede formatear
    ...
    if not df_view_detalle_ses.empty:
        ...
        try:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer: df_excel.to_excel(writer, index=False, sheet_name='Detalle_Sesiones')
...
# Intenta cargar datos. Si load_sesiones_data lanza una excepción (ej. por error de Secrets), Streamlit la mostrará.
try:
    df_sesiones_base = load_sesiones_data()
except ValueError as ve: # Capturar errores específicos lanzados por load_sesiones_data
    st.error(f"Error Crítico en la Carga de Datos: {ve}")
    st.stop()
except Exception as e: # Capturar cualquier otro error inesperado durante la carga
...
if df_sesiones_base is None or df_sesiones_base.empty:
    ...
    # Aquí se podría añadir un st.info con los processing_warnings si se implementó ese patrón
    st.stop()
...
# ... (resto de las llamadas a display_analisis_por_dimension) ...
st.markdown("---")
